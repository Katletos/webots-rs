use webots_bindings::{
    ...
    WbDistanceSensorType_WB_DISTANCE_SENSOR_LASER,
    WbDistanceSensorType_WB_DISTANCE_SENSOR_SONAR,
};
...
impl From<u32> for DistanceSensorType {
    ...
    fn from(other: u32) -> Self {
        match other {
            WbDistanceSensorType_WB_DISTANCE_SENSOR_GENERIC => {
                DistanceSensorType::Generic
            }
            WbDistanceSensorType_WB_DISTANCE_SENSOR_INFRA_RED => {
                DistanceSensorType::InfraLed
            }
            WbDistanceSensorType_WB_DISTANCE_SENSOR_SONAR => {
                DistanceSensorType::Sonar
            }
            WbDistanceSensorType_WB_DISTANCE_SENSOR_LASER => {
                DistanceSensorType::Laser
            }
            _ => unreachable!(),
        }
    }
